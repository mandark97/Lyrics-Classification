...
def main(count_vect, svc_conf, _run):
    ...
    w2v_model.build_vocab(x.split() for x in X)

    w2v_model.train(cleaned_text, total_examples=w2v_model.corpus_count, epochs=100)
    w2v_model.init_sims(replace=True)
    x_w2v = np.array([get_sentence_embedding(sentence) for sentence in X])
